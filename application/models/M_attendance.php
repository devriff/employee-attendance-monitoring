...
class M_attendance extends CI_Model {
  ...
  public function _get_todays_time_out_attendance($dateNow) {
    ...
    $query = $this->db
            ->select('*')
            ->from('dtr')
            ->join('employees', 'employees.emp_id = dtr.emp_id')
            ->where('dtr.date', $date)
            // ->where('dtr.att_type', 'time out')
            ->order_by('dtr.id', 'ASC')
            ->get();
    return $query->result();
  }
  ...
  public function _dtr($emp_id = FALSE){
    $query = $this->db
            ->select('dtr.emp_id, date, time_in, time_out')
            ->from('dtr')
            ->join('employees', 'employees.emp_id = dtr.emp_id')
            ->where('dtr.emp_id', $emp_id)
            ->limit(25)
            ->get();
    return $query->result();
  }
  ...
  public function _get_data(){
    $query = $this->db
            ->select('emp_id, date, GROUP_CONCAT(DISTINCT time_in SEPARATOR "") as t_in, GROUP_CONCAT(DISTINCT time_out SEPARATOR "") as t_out')
  ...
  public function _save_dtr($data = FALSE){
    $save_data['emp_id'] = $data->emp_id;
    $save_data['date'] = $data->date;
    $save_data['time_in'] = $data->t_in;
    $save_data['time_out'] = $data->t_out;
